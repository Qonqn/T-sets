...
const double root_space[4]={121.153,121.809,30.999,31.428};
...
long long space_index(long long index,vector<std::pair<double, double>>& trajectory,double x_min,double y_min,double x_max,double y_max){
    bool isExistPoint[4];
    for (int i =0; i < 4; i++) {
        isExistPoint[i] = 0;
    }
    ...
    //std::cout<<isExistPoint[0] <<" "<<isExistPoint[1]<<" "<<isExistPoint[2]<<" "<< isExistPoint[3]<<endl;
...
set<string> search_by_space(shared_ptr<ZRedisConnection> con,double x_min,double x_max,double y_min,double y_max,vector<pair<double,double>> query){
    set<string> keys;
    int unsatisfy_num=0;
    int satisfy_num=0;
    long long initial_index=space_index(1,query,x_min,y_min, x_max,y_max);
    ...
    RedisResult res;
    cout<<"space code is "<<index_str<<endl;
    ...
    for(int i=0;i<res.vecdata.size();i++){
        ...
        vector<string> key_split;
        Split(res.vecdata[i],"#",key_split);
        if(index_str.size()==key_split[0].size()){
    ...
    //cout<<"pc helps filter "<<unsatisfy_num<<endl;
...
set<string> Tbsimilar_trajectory_search(shared_ptr<ZRedisConnection> con,string traj_str,double Threshold){
    ...
    cout<<space_range[0]<<" "<<space_range[1]<<" "<<space_range[2]<<" "<<space_range[3]<<endl;
...
vector<double> index_to_space(long long index){
    vector<double> res;
    double x_min=root_space[0];
    double y_min=root_space[2];
    double x_max=root_space[1];
    double y_max=root_space[3];
...
double minDistTS(long long index,vector<pair<double,double>> query_traj){
...
double DistTT(shared_ptr<ZRedisConnection> con,string tid,vector<pair<double,double>> query_traj){
    ...
    RedisResult res;
    string cmd="hget "+tid+" points_str";
    con->ExecCmd(cmd,res);
    ...
    while(!spaceQueue.empty()){
        ...
        if(smallerSpaceQueue.empty()||smallerSpaceQueue.top().minsimilarity>=space.minsimilarity){
            ...
            for(int i=0;i<=3;i++){
                ...
                //cout<<childSpace.index<<" "<<childSpace.minsimilarity<<endl;
